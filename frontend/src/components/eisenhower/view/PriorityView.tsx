...
const useCustomSensors = () => {
    const pointerSensor = useSensor(PointerSensor, {
        activationConstraint: {
            distance: 4, // 5px 이상 이동 시 드래그 시작
            delay: 300, // 100ms 지연 후 드래그 시작
        },
    });
                                    ...
                                    <div className={cn('grid gap-2 md:mb-4 grid-cols-1')}>
                                        {(['Q1', 'Q2', 'Q3', 'Q4'] as Quadrant[]).map((q) => (
                                            <Droppable key={q} id={q}>
                                                ...
                                                >
                                                    <div
                                                        className="w-6 h-6 flex items-center justify-center rounded-[8px] text-sm font-semibold leading-none bg-blue text-neon-green shrink-0">
                                                        {q.replace('Q', '')}
                                                    </div>
                                                    {quadrantTitles[q]}
                                                </button>
