...
export default function PriorityMatrix() {
  ...
  const getColumnId = (taskId: string): string | undefined =>
    Object.keys(columns).find(col => columns[col].some(task => task.id === taskId))
  ...
  const handleDragStart = (event: any) => {
    ...
    const task = columnId ? columns[columnId].find(t => t.id === active.id) : null
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {Object.entries(columns).map(([colId, tasks]) => (
          <Column key={colId} id={colId} tasks={tasks} onDelete={handleDelete} onAdd={handleAdd} />
        ))}
      </div>
